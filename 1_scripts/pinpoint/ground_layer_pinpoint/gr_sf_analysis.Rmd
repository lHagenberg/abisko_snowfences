...
```{r read data}
vp_data <- read.csv("~/Project Green Arctic/3_snow_fences/1_data/1_sf_data_analysis/3_output/1_data_products/vp_pp_2023_w.csv")

meta_data <- read_csv("~/Project Green Arctic/3_snow_fences/1_data/1_sf_data_analysis/2_input/1_raw_data/sf_plot_data_2023.csv") %>% 
  mutate(habitat = case_when(habitat == 'D' ~ 'meadow',
                           habitat == 'E' ~ 'heath',
                           habitat == "X" ~ 'ridge')) 
  
plot_stats <- read_csv("~/Project Green Arctic/3_snow_fences/1_data/1_sf_data_analysis/3_output/1_data_products/plot_stats_2023.csv") %>% 
  mutate(habitat = case_when(habitat == 'D' ~ 'meadow',
                           habitat == 'E' ~ 'heath',
                           habitat == "X" ~ 'ridge'))
```
...
meta_nmds <- cbind(meta_data, vp_nmds) %>% 
  select(habitat:treatment, NMDS1, NMDS2)
```
...
write_csv(VAS.cov.change, "~/Project Green Arctic/3_snow_fences/1_data/1_sf_data_analysis/3_output/2_results/species_cover_dif_2023.csv")
```
...
plot_stats %>% 
  ...
  ggplot(aes(x = type, y = hits, color = treatment)) +
  geom_boxplot() +
...
#ggsave("~/Project Green Arctic/3_snow_fences/1_data/1_sf_data_analysis/3_output/3_plots/sf_type_cover_23.svg", height = 4, width = 7)
```
...
ggsave("~/Project Green Arctic/3_snow_fences/1_data/1_sf_data_analysis/3_output/3_plots/vaculi_hits_23.svg", height = 3, width = 4)
```