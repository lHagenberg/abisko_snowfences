...
```{r}
#| label: connect to OSF
rawData <- osf_retrieve_node("xw3hy")  
```
...
```{r}
#| label: total empetrum from sorted fraction
emp_biomass <- biomass %>% 
...
rawData %>%
  ...
  filter(name == "clean") %>%
  osf_mkdir("biomass") %>%
  osf_upload("2_data/2_clean/biomass/sf_biomass.csv")
```

```{r}
#| label: get shortened species names
...
```{r}
#| label: merge biomass subplot pinpoint and biomass data
...
rawData %>%
  osf_ls_files() %>%
  filter(name == "clean") %>%
  osf_upload("2_data/2_clean/sf_community.csv")
```
...
## leaf biomass
NOTE: depricated, see plot_level_bayes_stats
```{r}
