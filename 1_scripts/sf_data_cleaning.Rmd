...
rawdata %>%
  ...
  filter(name == "clean") %>%
  osf_mkdir("pinpoint") %>% 
  osf_upload("2_data/2_clean/pinpoint/vp_pp_2025_l.csv")
```
...
rawData %>%
  osf_ls_files() %>%
  filter(name == "clean") %>%
  osf_upload("2_data/2_clean/plot_stats_veg_2025.csv")
```
...
traits <- traits_no_chem %>%  # pool data by plot and species
  ...
  summarise(height = mean(height, na.rm = TRUE),
            dweight = mean(dweight, na.rm = TRUE), 
            fweight = mean(fweight, na.rm = TRUE),
            leafa = mean(leafa, na.rm = TRUE), 
...
rawData %>%
  ...
  filter(name == "clean") %>%
  osf_mkdir("traits") %>% 
  osf_upload("2_data/2_clean/traits/traits_w.csv")
```
...
rawData %>%
  ...
  filter(name == "clean") %>%
  osf_mkdir("traits") %>% 
  osf_upload("2_data/2_clean/traits/traits_l.csv")
```
...
rawData %>%
  ...
  filter(name == "clean") %>%
  osf_mkdir("traits") %>% 
  osf_upload("2_data/2_clean/traits/try_traits.csv")
```
...
```{r}
#| label: pull data from OSF
if (!file.exists("2_data/1_raw/soil/soil_chem.csv")) {
  if (!dir.exists("2_data/1_raw/soil")) {
    dir.create("2_data/1_raw/soil", recursive = TRUE)
  }
  ...
  rawData %>%
    ...
    filter(name == "soil") %>%
    osf_ls_files() %>%
    osf_download("2_data/1_raw/soil")
}
```

```{r}
#| label: wrangle data and upload to OSF
soil_chem <- read_csv("2_data/1_raw/soil/soil_chem.csv") %>%
  mutate(
    treat = recode(
      treat,
      "S" = "snowfence",
  ...
  rename(site = "plot") %>%
  mutate(plot = paste0(veg, site, treat), .before = "veg") %>%
  ...
  rename(habitat = "veg", treatment = "treat") %>%
  select(!identifier)

soil_chem %>%
  head(10)

write.csv(soil_chem, "2_data/2_clean/soil/soil_chem.csv")

rawData %>%
  osf_ls_files() %>%
  filter(name == "clean") %>%
  osf_mkdir("soil") %>%
  osf_upload("2_data/2_clean/soil/soil_chem.csv")
```
...
```{r}
#| label: pull data from osf
if (!file.exists("2_data/1_raw/clim/seasonal_soil_temperatures.csv")) {
  if (!dir.exists("2_data/1_raw/clim")) {
    dir.create("2_data/1_raw/clim", recursive = TRUE)
 }
  ...
  rawData %>%
    ...
    filter(name == "clim") %>%
    osf_ls_files() %>%
    osf_download("2_data/1_raw/clim")
}
```

```{r}
#| label: wrangel data and upload to osf
soil_temp <- read_csv("2_data/1_raw/clim/seasonal_soil_temperatures.csv") %>%
  select(!...1) %>%
  mutate(
    vegetation = recode(
      vegetation,
  ...
  mutate(site = substr(plot, 2, 2), .before = "season_class") %>%
  select(!plot)

head(soil_temp, 10)

write_csv(soil_temp, "2_data/2_clean/clim/soil_temp.csv")

rawdata %>%
  osf_ls_files() %>%
  filter(name == "clean") %>%
  osf_mkdir("clim") %>%
  osf_upload("2_data/2_clean/clim/soil_temp.csv")
```

# macroclimate
```{r}
#| label: import data
clim_2019 <- read_csv("2_data/1_raw/clim/MJA_clim_2019.csv")
clim_2020 <- read_csv("2_data/1_raw/clim/MJA_clim_2020.csv")
clim_2021 <- read_csv("2_data/1_raw/clim/MJA_clim_2021.csv")
clim_2022 <- read_csv("2_data/1_raw/clim/MJA_clim_2022.csv")
clim_2023 <- read_csv("2_data/1_raw/clim/MJA_clim_2023.csv")
clim_2024 <- read_csv("2_data/1_raw/clim/MJA_clim_2024.csv")
clim_2025 <- read_csv("2_data/1_raw/clim/MJA_clim_2025.csv")

clim <- bind_rows(clim_2019, clim_2020, clim_2021, clim_2022, clim_2023, clim_2024, clim_2025); rm(clim_2019, clim_2020, clim_2021, clim_2022, clim_2023, clim_2024, clim_2025)


write_csv(clim, "2_data/2_clean/clim/MJA_clim.csv")

```

```{r}
rawData %>% # macroclimatic data
  osf_ls_files() %>%
  filter(name == "clean") %>%
  osf_mkdir("clim") %>% 
  osf_upload("2_data/2_clean/clim/MJA_clim.csv")

rawData %>% # metadata
  osf_ls_files() %>%
  filter(name == "clean") %>%
  osf_mkdir("clim") %>% 
  osf_upload("2_data/1_raw/clim/MJA_clim_metadata.csv")
...
clim %>% 
  ...
  #group_by(year) %>% 
  summarise(
    tYear = mean(TA, na.rm  = TRUE),
    pYear = sum(PRECIP, na.rm = TRUE)/7,
  )
