...
library(vegan); library(pairwiseAdonis)
library(gllvm)
library(permute)
...
## functions
```{r}
# Pairwise PERMANOVA function (no installation needed!)
pairwise_permanova <- function(dist_matrix, factor, permutations = 999) {
  ...
  for(i in 1:(n-1)) {
    for(j in (i+1):n) {
      keep <- factor %in% c(levels[i], levels[j])
      sub_dist <- as.dist(as.matrix(dist_matrix)[keep, keep])
      sub_factor <- droplevels(factor[keep])
      
      result <- adonis2(sub_dist ~ sub_factor, permutations = permutations)
  ...
  comparisons$p_adjusted <- p.adjust(comparisons$p_value, method = "bonferroni")
  return(comparisons)
}
```
...
adonis2(as.matrix(.[, -c(1:4)]) ~ (habitat : treatment) + habitat + treatment, ., permu = 99999, method = "bray")
```
...
```{r}
# 1. Run NMDS on raw data (not distance matrix)
bm_nmds <- leaf_bm %>% 
...
# 2. Extract site scores (keep row names!)
site_nmds <- bm_nmds %>% 
  scores(display = "sites") %>%  # Don't use tidy = TRUE yet
  as.data.frame() %>% 
  rownames_to_column("plot_id")
...
meta_nmds <- site_nmds %>% 
  left_join(meta_data, by = "plot_id") %>%  # Fixed join
  select(plot_id, habitat, treatment, NMDS1, NMDS2) %>% 
  na.omit()
```

```{r}
# meta_nmds <- c("fen", "heath", "ridge") %>% 
# map_df(function(h) {
#   meta_sub <- meta_data %>% 
#   filter(habitat == h)
# 
# bm_nmds <- leaf_bm[rownames(leaf_bm) %in% meta_sub$plot_id, ] %>% 
#   metaMDS(distance = "bray") %>% 
#   scores(tidy = TRUE) %>% 
#   filter(score == "sites") %>% 
#   select(NMDS1, NMDS2)
...
ggplot() +
  ...
  scale_colour_manual(values = c("#917D53", "#667E4E", "#A8AA97")) +
  scale_fill_manual(values = c("#917D53", "#667E4E", "#A8AA97")) +
  geom_text(data = spec_nmds, aes(x = NMDS1, y = NMDS2, label = species), alpha = 0.5) +  # add the species labels
  #geom_text(data = data_scores, aes(x = NMDS1, y = NMDS2, label = plot), vjust=0) +  # add the site labels
  #coord_equal() +
  theme_classic() +
...
#### analysis
```{r PERMANOVA}
adonis2(leaf_bm ~ treatment * habitat, strata = meta_data$site_id, data = meta_data, method = "bray") 

x <- meta_data %>% 
  filter(habitat == "ridge")

y <- leaf_bm[rownames(leaf_bm) %in% x$plot_id, ] 

# 1. Calculate dispersion
dispersion <- vegdist(y, method = "bray") %>% 
  betadisper(x$treatment)

# 2. Test if dispersion is significantly different
permutest(dispersion)

# 3. If significant, see which groups differ in spread
TukeyHSD(dispersion)

# pairwise adonis
pairAdonis_results <- map_dfr(unique(meta_data$habitat), ~ {
  x <- meta_data %>% 
    filter(habitat == .x)  # ← Use .x instead of hab
  y <- leaf_bm[rownames(leaf_bm) %in% rownames(x), ]
  
  pairwise_result <- pairwise.adonis2(y ~ treatment, 
                                      #strata = meta_data$site_id,
                                      method = "euclidian", 
                                      data = x)
...
write.csv(pairAdonis_results, "3_output/2_statistics/community/pairwise_adonis_community.csv", row.names = FALSE)
```

```{r PERMANOVA meadow}
# per habitat
model_fit <- c("fen", "heath", "ridge") %>% 
map_df(function(h) {
  x <- meta_data %>% 
    filter(habitat == h)
  ...
  fit <- adonis2(y ~ treatment, 
                 data = x, method = "bray", 
                 permutations = 9999) %>% 
    tidy() %>% 
    mutate(habitat = h, .before = "term")
})

write_csv(model_fit, "3_output/2_statistics/community/nmds_permanova.csv")
