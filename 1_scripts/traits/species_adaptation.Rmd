...
#### manual
```{r}
# fixed averages (CWM using species trait means)
species_traits <- traits_l %>% 
  group_by(spec, trait) %>% 
...
# specific averages (CWM using plot level traits)
spec_avg <- community %>% 
  filter(part == "leaf") %>% 
  select(plot, spec, hits, biomass) %>% 
  left_join(traits, by = c("plot", "spec")) %>%  
  mutate(height = biomass * height, 
            LDMC = biomass * LDMC, 
            SLA = biomass * SLA, 
            wN = biomass * wN, 
...
ITV <- bind_rows(fix_avg, spec_avg) %>% 
  ...
  mutate(ITV = abs(local - landscape)) %>% 
