...
```{r}
# extract PCA scores
pca_scores <- scores(plot_pca, display = "sites", choices = 1:8) %>% 
  as.data.frame()
...
all(rownames(pca_scores) == rownames(pca_meta_data)) # PCA scores and meta data match!

# PERMANOVA on PCA scores [how many axes?]
adonis2(pca_scores ~ treatment * habitat, data = pca_meta_data, method = "euclidian")

pairAdonis_results <- map_dfr(unique(pca_meta_data$habitat), ~ {
  x <- pca_meta_data %>% 
    filter(habitat == .x)  # ← Use .x instead of hab
  y <- pca_scores[rownames(x), ]
